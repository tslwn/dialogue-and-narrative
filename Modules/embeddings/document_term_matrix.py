...
class DocumentTermMatrix(AbstractTermVectors):
    ...
    def __init__(
        ...
        self._vectorizer = CountVectorizer(ngram_range=ngram_range)
        self._vectorizer.fit(documents)
        self._matrix = self._vectorizer.transform(documents)
        self._array = self._matrix.toarray()
    ...
    def terms(self) -> Generator[str, None, None]:
        for term in self._vectorizer.vocabulary_.keys():
            yield term

    def term_index(self, term: str) -> int:
        return self._vectorizer.vocabulary_[term]

    def term_vector(self, term: str) -> NDArray[float64]:
        return self._matrix.getcol(self.term_index(term)).toarray().flatten()
    ...
    def term_documents(
        ...
        for document_index, document_count in enumerate(self.term_vector(term)):
            ...
            if document_count > 0:
                count += 1
                yield self._documents[document_index]

    def document_vector(self, document_index: int):
        """Get the vector for a document."""
        return self._matrix.getrow(document_index).toarray().flatten()
